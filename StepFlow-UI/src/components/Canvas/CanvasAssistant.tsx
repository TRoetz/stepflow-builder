...
export function CanvasAssistant() {
  ...
  const focusedNodeName = useAiAssistantStore((s) => s.context.selectedNodeName);
  const hasFocusContext = useAiAssistantStore((s) => s.context.selectedNodeDescription !== null);
      ...
      <div className="bg-gray-900/95 backdrop-blur-sm border border-gray-700/50 rounded-xl shadow-2xl overflow-hidden">
        ...
        {!isCollapsed && (
          <>
            ...
            {/* Quick prompts for the focused state */}
            {hasFocusContext && focusedNodeName && !isTyping && (
              <div className="flex flex-wrap gap-1.5 px-4 pb-2">
                {['How do I use this state?', 'Check my inputs & outputs', 'Show me correct syntax'].map((q) => (
                  <button
                    key={q}
                    type="button"
                    onClick={() => sendPrompt(q)}
                    className="px-2.5 py-1 rounded-full text-[11px] bg-indigo-600/15 border border-indigo-500/30 text-indigo-300 hover:bg-indigo-600/30 hover:text-indigo-200 transition-colors"
                  >
                    {q}
                  </button>
                ))}
